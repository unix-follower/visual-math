...
[[nodiscard]] bool parse_scene_kind(std::string_view text, OpenGLSceneKind& scene_kind) {
	if (text == "gradient-triangle") {
...
bool parse_opengl_launch_options(
	std::span<const std::string_view> arguments,
	OpenGLLaunchOptions& options,
	std::string& error_message) {
	options = OpenGLLaunchOptions{};
	error_message.clear();
	...
	for (const std::string_view argument : arguments) {
		...
		if (const auto value = argument.substr(argument.find('=') + 1); argument.starts_with("--scene-kind=")) {
			if (!parse_scene_kind(value, options.scene_kind)) {
				error_message = "Invalid value for --scene-kind.";
				return false;
			}
			continue;
		}
