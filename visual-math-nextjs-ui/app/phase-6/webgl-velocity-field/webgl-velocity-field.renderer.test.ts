...
describe("webgl-velocity-field.renderer", () => {
  it("renders persistent velocity advection across frames and releases resources", () => {
    const runtime = createVelocityFieldRuntime()
    ...
    renderWebGlVelocityFieldScene(canvas, runtime as never, DEFAULT_WEBGL_VELOCITY_FIELD_SCENE, 0.5)

    expect(runtime.context.uniform1f).toHaveBeenNthCalledWith(1, runtime.phaseLocation, 0.25)
    expect(runtime.context.uniform1f).toHaveBeenNthCalledWith(2, runtime.swirlLocation, 0.62)
    expect(runtime.context.uniform1f).toHaveBeenNthCalledWith(3, runtime.shearLocation, 0.38)
    expect(runtime.context.uniform1f).toHaveBeenNthCalledWith(4, runtime.injectionLocation, 0.74)
    expect(runtime.context.uniform1f).toHaveBeenNthCalledWith(5, runtime.dissipationLocation, 0.56)
    expect(runtime.context.uniform1f).toHaveBeenNthCalledWith(6, runtime.mixLocation, 0.64)
    expect(runtime.context.uniform1f).toHaveBeenNthCalledWith(7, runtime.phaseLocation, 0.5)
    ...
    expect(releaseWebGlVelocityFieldResources(runtime as never)).toBe(true)
    expect(runtime.context.deleteBuffer).toHaveBeenCalledWith(runtime.evolveQuadBuffer)
    ...
    expect(releaseWebGlVelocityFieldResources(runtime as never)).toBe(false)
  })
})

function createVelocityFieldRuntime() {
  ...
  const shearLocation = { id: "u_shear" } as unknown as WebGLUniformLocation
  ...
  const context = {
    ...
    getUniformLocation: jest.fn((program: unknown, name: string) => {
      if (program === evolveProgram) {
        ...
        if (name === "u_shear") return shearLocation
        if (name === "u_injection") return injectionLocation
        return dissipationLocation
      }
