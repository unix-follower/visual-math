...
describe("webgl-interactive-dye.renderer", () => {
  it("renders dye injection state and releases cached resources", () => {
    const runtime = createInteractiveDyeRuntime()
    ...
    expect(runtime.context.uniform1f).toHaveBeenCalledWith(runtime.swirlLocation, 0.54)
    ...
    expect(runtime.context.uniform1f).toHaveBeenCalledWith(runtime.speedLocation, 1.02)
    expect(runtime.context.uniform1f).toHaveBeenCalledWith(runtime.injectionXLocation, 0.28)
    expect(runtime.context.uniform1f).toHaveBeenCalledWith(runtime.injectionYLocation, 0.62)
    expect(runtime.context.uniform1f).toHaveBeenCalledWith(runtime.obstacleXLocation, 0.68)
    expect(runtime.context.uniform1f).toHaveBeenCalledWith(runtime.obstacleYLocation, 0.42)
    expect(runtime.context.uniform1f).toHaveBeenCalledWith(runtime.obstacleRadiusLocation, 0.16)
    ...
    expect(releaseWebGlInteractiveDyeResources(runtime as never)).toBe(true)
    ...
    expect(releaseWebGlInteractiveDyeResources(runtime as never)).toBe(false)
  })
})

function createInteractiveDyeRuntime() {
  ...
  const obstacleXLocation = { id: "u_obstacleX" } as unknown as WebGLUniformLocation
  const obstacleYLocation = { id: "u_obstacleY" } as unknown as WebGLUniformLocation
  const obstacleRadiusLocation = { id: "u_obstacleRadius" } as unknown as WebGLUniformLocation
  ...
  const context = {
    ...
    getUniformLocation: jest.fn((program: unknown, name: string) => {
      if (program === evolveProgram) {
        ...
        if (name === "u_obstacleX") return obstacleXLocation
        if (name === "u_obstacleY") return obstacleYLocation
        if (name === "u_obstacleRadius") return obstacleRadiusLocation
        return injectionStrengthLocation
      }

      if (name === "u_texture") return compositeTextureLocation
      if (name === "u_mix") return mixLocation
      if (name === "u_injectionStrength") return injectionStrengthLocation
      return obstacleRadiusLocation
    }),
    bindBuffer: jest.fn(),
    bufferData: jest.fn(),
    bindTexture: jest.fn(),
