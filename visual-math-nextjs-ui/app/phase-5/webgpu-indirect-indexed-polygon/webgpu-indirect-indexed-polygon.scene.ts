...
export function createWebGpuIndirectIndexedPolygonSceneResources(
  ...
  const shaderModule = runtime.device.createShaderModule({
    label: "visual-math-nextjs-webgpu-indirect-indexed-polygon-shader",
    code: SHADER_SOURCE,
  })
  const pipeline = runtime.device.createRenderPipeline({
    label: "visual-math-nextjs-webgpu-indirect-indexed-polygon-pipeline",
    layout: "auto",
    vertex: {
      module: shaderModule,
  ...
  const vertexBuffer = runtime.device.createBuffer({
    label: "visual-math-nextjs-webgpu-indirect-indexed-polygon-vertices",
    size: MAX_VERTEX_COUNT * VERTEX_STRIDE,
    usage: GPUBufferUsage.VERTEX | GPUBufferUsage.COPY_DST,
  })
  const indexBuffer = runtime.device.createBuffer({
    label: "visual-math-nextjs-webgpu-indirect-indexed-polygon-indices",
    size: MAX_INDEX_COUNT * UINT16_BYTES,
    usage: GPUBufferUsage.INDEX | GPUBufferUsage.COPY_DST,
  })
  const indirectBuffer = runtime.device.createBuffer({
    label: "visual-math-nextjs-webgpu-indirect-indexed-polygon-indirect",
