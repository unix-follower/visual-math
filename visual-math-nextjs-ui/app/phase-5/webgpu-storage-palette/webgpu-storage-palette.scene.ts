...
export function createWebGpuStoragePaletteSceneResources(
  ...
  const shaderModule = runtime.device.createShaderModule({
    label: "visual-math-nextjs-webgpu-storage-palette-shader",
    code: SHADER_SOURCE,
  })
  const pipeline = runtime.device.createRenderPipeline({
    label: "visual-math-nextjs-webgpu-storage-palette-pipeline",
    layout: "auto",
    vertex: {
      module: shaderModule,
