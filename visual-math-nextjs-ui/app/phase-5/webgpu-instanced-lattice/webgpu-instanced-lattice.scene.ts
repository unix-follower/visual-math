...
export function createWebGpuInstancedLatticeSceneResources(
  ...
  const shaderModule = runtime.device.createShaderModule({
    label: "visual-math-nextjs-webgpu-instanced-lattice-shader",
    code: SHADER_SOURCE,
  })
  const pipeline = runtime.device.createRenderPipeline({
    label: "visual-math-nextjs-webgpu-instanced-lattice-pipeline",
    layout: "auto",
    vertex: {
      module: shaderModule,
  ...
  const meshBuffer = runtime.device.createBuffer({
    label: "visual-math-nextjs-webgpu-instanced-lattice-mesh",
    size: BASE_MESH_DATA.byteLength,
    usage: GPUBufferUsage.VERTEX | GPUBufferUsage.COPY_DST,
  })
  const instanceBuffer = runtime.device.createBuffer({
    label: "visual-math-nextjs-webgpu-instanced-lattice-instances",
