...
export function createWebGpuComputeRippleSceneResources(
  ...
  const computeShader = runtime.device.createShaderModule({
    label: "visual-math-nextjs-webgpu-compute-ripple-compute-shader",
    code: COMPUTE_SHADER,
  })
  const renderShader = runtime.device.createShaderModule({
    label: "visual-math-nextjs-webgpu-compute-ripple-render-shader",
    code: RENDER_SHADER,
  })
  const computePipeline = runtime.device.createComputePipeline({
    label: "visual-math-nextjs-webgpu-compute-ripple-compute-pipeline",
    layout: "auto",
    compute: { module: computeShader, entryPoint: "computeMain" },
  })
  const renderPipeline = runtime.device.createRenderPipeline({
    label: "visual-math-nextjs-webgpu-compute-ripple-render-pipeline",
    layout: "auto",
    vertex: {
      module: renderShader,
