...
export function createWebGpuSamplerWaveSceneResources(
  ...
  const shaderModule = runtime.device.createShaderModule({
    label: "visual-math-nextjs-webgpu-sampler-wave-shader",
    code: SHADER_SOURCE,
  })
  const pipeline = runtime.device.createRenderPipeline({
    label: "visual-math-nextjs-webgpu-sampler-wave-pipeline",
    layout: "auto",
    vertex: {
      module: shaderModule,
