...
export function webGpuFoundationSummary(
  ...
  return `WebGPU foundation is ${runtimeStatus.toLowerCase()} with a pipeline-backed triangle draw targeting ${formatLabel}. The viewport clears to ${formatClearColor(scene)} and overlays a triangle at ${Math.round(scene.triangleScale * 100)}% scale with accent ${scene.accent.toFixed(2)} so later Phase 5 slices can grow from real shaders and vertex buffers instead of a clear pass alone.`
}
