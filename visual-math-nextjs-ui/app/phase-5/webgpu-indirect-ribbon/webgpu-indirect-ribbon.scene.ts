...
export function createWebGpuIndirectRibbonSceneResources(
  ...
  const shaderModule = runtime.device.createShaderModule({
    label: "visual-math-nextjs-webgpu-indirect-ribbon-shader",
    code: SHADER_SOURCE,
  })
  const pipeline = runtime.device.createRenderPipeline({
    label: "visual-math-nextjs-webgpu-indirect-ribbon-pipeline",
    layout: "auto",
    vertex: {
      module: shaderModule,
