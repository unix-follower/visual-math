...
export type WebGpuIndexedPolygonSceneResources = {
  readonly pipeline: GPURenderPipeline
  readonly vertexBuffer: GPUBuffer
  readonly indexBuffer: GPUBuffer
}

export function createWebGpuIndexedPolygonSceneResources(
  runtime: WebGpuCanvasRuntime,
): WebGpuIndexedPolygonSceneResources {
  const shaderModule = runtime.device.createShaderModule({
    label: "visual-math-nextjs-webgpu-indexed-polygon-shader",
    code: SHADER_SOURCE,
  })
  const pipeline = runtime.device.createRenderPipeline({
    label: "visual-math-nextjs-webgpu-indexed-polygon-pipeline",
    layout: "auto",
    vertex: {
      module: shaderModule,
...
export function buildWebGpuIndexedPolygonVertexData(
  scene: WebGpuIndexedPolygonScene,
): Float32Array {
  const vertexData: number[] = []
  const angleOffset = (scene.rotation * Math.PI) / 180
  const centerColor = [
    0.26 + scene.intensity * 0.48,
    0.2 + (1 - scene.green) * 0.34,
    0.24 + (1 - scene.blue) * 0.28,
    1,
  ]
  ...
  for (let index = 0; index < scene.sides; index += 1) {
    ...
    const color = [
      0.18 + scene.intensity * (0.24 + blend * 0.32),
      0.22 + (1 - scene.green) * (0.18 + (1 - blend) * 0.24),
      0.28 + (1 - scene.blue) * (0.16 + blend * 0.2),
      1,
    ]
...
export function buildWebGpuIndexedPolygonIndexData(scene: WebGpuIndexedPolygonScene): Uint16Array {
...
export function webGpuIndexedPolygonVertexCount(scene: WebGpuIndexedPolygonScene): number {
  return indexedPolygonVertexCount(scene)
}

export function webGpuIndexedPolygonIndexCount(scene: WebGpuIndexedPolygonScene): number {
  return indexedPolygonIndexCount(scene)
}
